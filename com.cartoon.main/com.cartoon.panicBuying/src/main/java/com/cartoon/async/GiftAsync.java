...
public class GiftAsync {
     ...
     */
    @Async
    public void rushAsync(String extime)throws SellOutException {
       ...
       //String extime = DateUtil.data2str(date, DateUtil.PATTERN_YYYYMMDDHH);
        ...
        if (token!=null) {
            //如果还有令牌
            //取出用户
            phone = (String)redisUtil.rifhtPop(GiftEmun.userQueue.getName()+extime);
            //去redis拿红包
            Gift giftData = (Gift) redisUtil.hmget(GiftEmun.rushPurchaseTime.getName() + extime).get(token.getId());
            ...
            giftData.setPhone(phone);
            map.put(phone,giftData);
